c Changes by I.V.L.: REAL->REAL*8, COMPLEX->COMPLEX*16, cmplx(...)->cmplx(...,8)
...
c                            ** recurrence ( Eq. R19 )
      ...
      ZETNM1 = CMPLX( PSINM1, CHINM1, 8 )
      ZETN   = CMPLX( PSIN, CHIN, 8 )
...
c     .. Local Arrays ..
     ...
     &          RECIP( MAXRCP )
      COMPLEX*16   C( MAXTRM ), CM( MAXTRM ), CS( MAXTRM ), D( MAXTRM ),
     &          DM( MAXTRM ), DS( MAXTRM )
c     ..
c     .. External Subroutines ..
...
c     .. Array Arguments ..
      ...
      REAL*8      XMU( * )
      COMPLEX*16   A( * ),B( * ),S1( * ),S2( * ),TBACK( * ),TFORW( * )
c     ..
c     .. Local Scalars ..
...
c                                                       ** Eq. R/A.5
      A( 1 ) = CMPLX( 0.0D0, TWOTHR*( 1.0D0 - 0.2D0*XX**2.0D0 ), 8 ) /
     &         CMPLX( 1.0D0 - 0.5D0*XX**2, TWOTHR*XX**3.0D0, 8 )
c                                                       ** Eq. R/A.6
      B( 1 ) = CMPLX( 0.0D0, - ( 1.0D0 - 0.1D0*XX**2.0D0 ) / 3.0D0, 8) /
     &         CMPLX( 1.0D0 + 0.5D0*XX**2, - XX**3.0D0 / 3.0D0, 8)
c                                                       ** Eq. R/A.7,8
      A( 2 ) = CMPLX( 0.0D0,   XX**2.0D0 / 30.0D0, 8)
      B( 2 ) = CMPLX( 0.0D0, - XX**2.0D0 / 45.0D0, 8)
c                                                       ** Eq. R/A.9
...
c     ..
      ...
      CIORSQ = CIOR**2
      CTMP   = CMPLX( 0.0D0, TWOTHR, 8 )*( CIORSQ - 1.0 )
